...
public class VibiResponseStoreAPI implements IVibiStoreAPI  { // WARNING: Change this name class for only 1 DB name only

    private static volatile VibiResponseStoreAPI instance;
    private Connection connection;
    private final String dbPath;
    private static final String BASE_DIR = Constants.STORE_API_DEFAULT_PATH; // WARNING: Change this BASE_DIR

    // Private constructor to ensure singleton pattern
    private VibiResponseStoreAPI() throws SQLException {
    ...
    /**
     * Get instance of VibiResponseStoreContextAPI (thread-safe double-checked locking)
     * Database name will automatically be set to the class name (e.g., "VibiResponseStoreContextAPI.db")
     * 
     * @return Unique instance of the class
     * @throws SQLException If error occurs during database initialization
     */
    public static VibiResponseStoreAPI getInstance() throws SQLException {
        if (instance == null) {
            synchronized (VibiResponseStoreAPI.class) {
                if (instance == null) {
                    instance = new VibiResponseStoreAPI();
                }
            }
        }
        return instance;
    ...
    private void createDefaultTable() throws SQLException {
        try (Statement stmt = connection.createStatement()) {
            stmt.execute("CREATE TABLE IF NOT EXISTS " + Constants.STORE_API_DEFAULT_TABLE_NAME + " (" +
                    "ID TEXT PRIMARY KEY, " +
                    "VALUE TEXT)");
        }
    }
    ...
    public void write(String id, String value) throws SQLException {
        String sql = "INSERT OR REPLACE INTO " + Constants.STORE_API_DEFAULT_TABLE_NAME + " (ID, VALUE) VALUES (?, ?)";
    ...
    public String read(String id) throws SQLException {
        String sql = "SELECT VALUE FROM " + Constants.STORE_API_DEFAULT_TABLE_NAME + " WHERE ID = ?";
        try (var pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, id);
    ...
    public boolean delete(String id) throws SQLException {
        String sql = "DELETE FROM " + Constants.STORE_API_DEFAULT_TABLE_NAME + " WHERE ID = ?";
        try (var pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, id);
    ...
    public java.util.List<String> selectAllIds() throws SQLException {
        ...
        String sql = "SELECT ID FROM " + Constants.STORE_API_DEFAULT_TABLE_NAME;
